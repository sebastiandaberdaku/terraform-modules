...
variable "attach_policy" {
  description = "Controls if S3 bucket should have bucket policy attached (set to `true` to use value of `policies` as bucket policy)"
  type        = bool
  default     = false
}
...
variable "policies" {
  description = "(Optional) A list of valid bucket policy JSON documents. The module will merge them into a single policy before applying it to the S3 bucket. Note that if the policy document is not specific enough (but still valid), Terraform may view the policy as constantly changing in a terraform plan. In this case, please make sure you use the verbose/specific version of the policy. For more information about building AWS IAM policy documents with Terraform, see the AWS IAM Policy Document Guide."
  type        = list(string)
  default     = null
}
